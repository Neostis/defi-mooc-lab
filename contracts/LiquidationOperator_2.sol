...
contract LiquidationOperator_2 is IUniswapV2Callee {
    ...
    // IUniswapV2Pair immutable uniswapV2Pair_USDC_USDT; // Pool3
    ...
    address constant liquidationNewTarget = 0x63f6037d3e9d51ad865056BF7792029803b6eEfD;
    ...
    constructor() {
        ...
        // uniswapV2Pair_USDC_USDT = IUniswapV2Pair(uniswapV2Factory.getPair(address(USDC), address(USDT))); // Pool3
    ...
    function operate(uint256 _debtAmountUSDT) external {
        ...
        debt_USDT = _debtAmountUSDT;
        uniswapV2Pair_WBTC_USDT.swap(0, debt_USDT, address(this), "$");

        // uniswapV2Pair_WETH_USDT.swap(0, _debtAmountUSDT, address(this), "$");
    ...
    function uniswapV2Call(
        ...
        // assert(msg.sender == address(uniswapV2Pair_USDC_USDT));
        (uint256 reserve_WBTH_Pool1, uint256 reserve_USDT_Pool1, ) = uniswapV2Pair_WBTC_USDT.getReserves(); // Pool1
        (uint256 reserve_WBTC_Pool2, uint256 reserve_WETH_Pool2, ) = uniswapV2Pair_WBTC_WETH.getReserves(); // Pool2

        // (uint256 reserve_USDC_Pool3, uint256 reserve_USDT_Pool3, ) = uniswapV2Pair_USDC_USDT.getReserves(); // Pool3
        ...
        uint repay_WBTC = getAmountIn(debtToCover, reserve_WBTH_Pool1, reserve_USDT_Pool1);
        console.log('repay_WBTC : ',repay_WBTC);

        WBTC.transfer(address(uniswapV2Pair_WBTC_USDT), repay_WBTC);

        // 2.4 convert to WBTC/ETH pool
        ...
        console.log('amountOut_WETH : ',amountOut_WETH);
        uniswapV2Pair_WBTC_WETH.swap(0, amountOut_WETH, address(this), "");
