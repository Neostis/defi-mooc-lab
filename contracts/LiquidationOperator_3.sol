...
contract LiquidationOperator_3 is IUniswapV2Callee {
    ...
    IUniswapV2Pair immutable uniswapV2Pair_WETH_USDC; // Pool3
    ...
    function operate(uint256 _debtAmountUSDC) external {
        ...
        (
            ...
            ltv,
            healthFactor
        ) = lendingPool.getUserAccountData(liquidationNewTarget);
        ...
        debt_USDC = _debtAmountUSDC;
        uniswapV2Pair_WETH_USDC.swap(debt_USDC, 0, address(this), "$");

        // uniswapV2Pair_WETH_USDT.swap(0, _debtAmountUSDC, address(this), "$");
    ...
    function uniswapV2Call(
        ...
        lendingPool.liquidationCall(address(WETH), address(USDC), liquidationNewTarget, debtToCover, false);
        ...
        uint repay_WETH = getAmountIn(debtToCover, reserve_WETH_Pool3, reserve_USDC_Pool3);
