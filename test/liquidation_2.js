...
describe("Liquidation", function () {
  ...
  it("operate 2000 USDT", async function () {
    ...
    const liquidatedAddress =
      "0x00000000000000000000000059ce4a2ac5bc3f5f225439b2993b86b42F6d3e9f".toLowerCase();
    ...
    console.log("Profit of 2000 USDT is ", utils.formatEther(profit), "ETH");

    expect(profit.gt(BigNumber.from(0)), "not profitable").to.be.true;
  });

  it("operate 5000 USDT", async function () {
    await network.provider.request({
      method: "hardhat_reset",
      params: [
        {
    ...
    const profit = afterLiquidationBalance.sub(beforeLiquidationBalance);
    console.log("Profit of 5000 USDT is ", utils.formatEther(profit), "ETH");

    expect(profit.gt(BigNumber.from(0)), "not profitable").to.be.true;
  });

  it("operate 10000 USDT", async function () {
    await network.provider.request({
      method: "hardhat_reset",
      params: [
        {
    ...
    const profit = afterLiquidationBalance.sub(beforeLiquidationBalance);
    console.log("Profit of 10000 USDT is ", utils.formatEther(profit), "ETH");
